...
export const createLead = async (request, response) => {
  try {
    ...
    const assignedTo = assignLeadToUser(users);
    ...
    await Activity.create({
      lead: lead._id,
      message: `Lead created and assigned to ${
...
export const getMyLeads = async (request, response) => {
  try {
    const { status, search } = request.query;
    let filter = { assignedTo: request.userId }; // ← only their leads
